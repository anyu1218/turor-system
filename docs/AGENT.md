...
- `docs/AGENT.md`：所有 AGENT 必须遵守的规范文档，涵盖工作规则、Commit 规范等
- `plan/详细短期计划/README.md`：短期计划的文件命名规范
- `progress/每日完成进度/README.md`：每日进度的文件命名与内容规范
- `README.md`：项目整体结构与功能概览

## 文档更新规则

- 完成一个短期计划（Pxx）后：在 `plan/短期计划总结/` 下创建对应的总结文件
- 每日工作结束后：在 `progress/每日完成进度/` 下创建当日的进度文件（YYYY-MM-DD.md）
- 更新进度总览：若项目阶段发生变化，同步更新 `progress/进度总览.md`
- 更新周目标：每周初在 `progress/进度总览.md` 的"本周目标"中填写本周计划

## Necessary Habits

- 涉及时间、今天、明天等表述时，按 Asia/Shanghai 理解。
- 全局记忆只记长期稳定、以后会反复用到的事实。
- 详细步骤不写进记忆，避免记忆臃肿。
...
- type: `fix` / `feat` / `docs` / `style` / `refactor` / `test` / `chore` 等
- scope: 模块名，如 `login`、`appointment`、`user` 等
- subject: 简短描述，用中文

### Body 规范

- **改动少**（少量文件、微调）：不写 body，一行搞定
  - 示例：`fix(login): 修复登录时验证码过期不刷新的问题`

- **改动多**（多文件、新功能、重构）：写结构化 body
  - 示例：
